...
export async function generateClientApp(appDir: string) {
    ...
    await copyAssets(join(import.meta.dirname, '../src', assetsForTemplateDir), generatedAppDir, pages)
...
async function renamePages(dir: string) {
    ...
    const pages = await Promise.all(files.filter(file=>{
        return file.toString().endsWith('page.tsx')
    }).map(async(file)=>{
        const dest = join(dir, dirname(file.toString()),`index${extname(file.toString())}`)
...
async function copyAssets(assetsDir: string, projectDir: string, pages: string[]) {
    ...
    await Promise.all(pages.map(async (page)=>{
        ...
        await replaceInFile(join(pageDir, 'index.html'), `/react.dom.render.tsx`, `${relDir===''?'':`/${relDir}`}/react.dom.render.tsx`)
        await copy(join(assetsDir, 'react.dom.render.tsx'), join(pageDir, 'react.dom.render.tsx'))
    }))
...
async function replaceInFile(filePath: string, oldText: string, newText: string) {
